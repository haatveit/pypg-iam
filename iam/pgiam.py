...
class Db(object):
    ...
    def __init__(self, engine: sqlalchemy.engine.Engine, config: dict = {}) -> None:
        ...
        self.tables = IamTables(
            ...
            audit_log_relations=self.meta.tables["audit_log_objects"],
        )
    ...
    def capabilities_http_sync(
        ...
        existing_names = []
        with session_scope(self.engine, session_identity) as session:
            results = session.execute(QUERY_CAPABILITIES_HTTP_LIST).fetchall()
        ...
        with session_scope(self.engine, session_identity) as session:
            for capability in capabilities:
                ...
                if capability.get("capability_name") in updates:
                    session.execute(CAPABILITIES_HTTP_UPDATE_QUERY, capability)
                elif capability.get("capability_name") in inserts:
                    session.execute(CAPABILITIES_HTTP_INSERT_QUERY, capability)
            if deletes:
                session.execute(
                    CAPABILITIES_HTTP_DELETE_QUERY, {"deletes": list(deletes)}
                )
    ...
    def capabilities_http_grants_sync(
        ...
        with session_scope(self.engine, session_identity) as session:
            for grant in grants:
                exists = session.execute(GRANTS_EXISTS_QUERY, grant).fetchone()[0]
                input_keys = grant.keys()
                ...
                if exists:
                    session.execute(GRANTS_UPDATE_QUERY, grant)
                    ...
                    session.execute(GRANTS_INSERT_QUERY, grant)
                    # get current grant_id from name
                    curr_grant_id = session.execute(
                        GRANTS_GET_ID_FROM_NAME_QUERY,
                        {"name": grant["capability_grant_name"]},
